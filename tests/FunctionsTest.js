...
describe('Add chocolates of differenent color - addChocolates', function () {
  ...
  const addChocolates = (candies, color, count) => {
    if (count <= 0) {
...
describe('Remove chocolates from the top of the dispenser - removeChocolates', function () {
  const removeChocolates = (candies, count) =>{
    if (count<=0){
      return 'Number cannot be zero/negative';
    }
...
describe('Dispense chocolates - dispenseChocolates', function () {
  const dispenseChocolates = (candies, count) => {
    if (count <= 0) {
      return 'Number cannot be zero/negative';
    }
    if(count > candies.length){
  ...
  it('Dispense 6 chocolates from the bottom of the dispenser, return them in an array.', function () {
    ...
    let expectedResult = ['silver', 'purple', 'silver', 'red', 'red', 'blue'].reverse();
...
describe('Dispense chocolates of different color - dispenseChocolatesOfColor', function () {
  const dispenseChocolatesOfColor = (candies, count, color) => {
    if(count<=0){
      return 'Number cannot be zero/negative'
    }
    if(count>candies.length){
    ...
    const chocolatesOfColor = candies.filter((candy) => candy === color);
    return chocolatesOfColor.slice(0, count);
  };
...
describe('Display number of chocolates of each color - noOfChocolates', function () {
  const noOfChocolates = (candies) => {
    const colorCounts = {};
    candies.forEach((candy) => {
      if (!colorCounts[candy]) {
        colorCounts[candy] = 0;
      }
      colorCounts[candy]++;
    });
...
describe('Sort chocolates based on count in each color - sortChocolateBasedOnCount', function () {
  const sortChocolateBasedOnCount = (candies) => {
    const colorCounts = {};
    candies.forEach((candy) => {
      if (!colorCounts[candy]) {
        colorCounts[candy] = 0;
  ...
  const sortedColors = Object.keys(colorCounts).sort((a, b) => {
    ...
    if (colorCounts[b] === colorCounts[a]) {
  ...
  sortedColors.forEach(color => {
    sortedCandies.push(...Array.from({ length: colorCounts[color] }, () => color));
  });
...
describe('Change the color of chocolates - changeChocolateColor', function () {
  ...
  const changeChocolateColor = (chocolates, color, finalColor) => {
    ...
    const count = chocolates.filter(choc => choc === color).length; 
    const updatedChocolates = chocolates.map(choc => (choc === color ? finalColor : choc)); 
    return [count, updatedChocolates];
  };
...
// uncomment the entire code below -- only if you have completed all the progressions:
 ...
 describe("Challenge 1: Fresh Pickings - ", function(){
  const removeChocolateOfColor = (candies, color) => {
    if (candies.length === 0) {
      return [];
    }
    const index = candies.findIndex(choc => choc === color); 
    if (index !== -1) {
  ...
  describe("Challenge 2: Rainbow and sunshine! - ", function(){
    const dispenseRainbowChocolates=(number) =>{
      const colorCounts={};
      number.forEach(color=>{
        colorCounts[color]=(colorCounts[color]||0)+1;
        });
        let rainbowCount=0;
        Object.values(colorCounts).forEach(count=>{
          rainbowCount+=Math.floor(count/3);
      });
      return rainbowCount;
     };
   ...
   it("", function () {
     ...
     //blue and silver can make up a rainbow chocolate
